...
data = hdf5storage.loadmat('raw/output/01.mat', variable_names={'csi_serial', 'frame'})
csi_data = torch.zeros(1, 30*5, 3, 3)
csi_data[0,:,:,: ]= torch.from_numpy(data['csi_serial']).type(torch.FloatTensor).reshape(-1, 3, 3) # view
frame = data['frame']
# frame = frame[...,::-1]
